...
for ind = 1:nr_regr_stim+1
        ...
        for ind_v = 1:size(B_temp,1)
            if (B_temp(ind_v))
                ...
                if fPET.Y.incomplete == 1
                    ...
                    x(ind_rem) = [];
                    temp = (fPET.tvec(end) + fPET.framelength/2)*fPET.quant.tstar;
                    [~,tstar] = min(abs(fPET.tvec - temp));
                else
    ...
    if (fPET.quant.in.type == 2) || (fPET.quant.in.type == 3)
        R.d_re = zeros(size(B.bl.d_re));
        if ind == 1
            R.h.fname = fullfile(fPET.dir.result, 'b2_baseline_Ki_ref.nii');
        else
            R.h.fname = fullfile(fPET.dir.result, sprintf('b%i_%s_Ki_ref.nii', ind+1, fPET.glm.X.name{ind+1}));
        end
        for ind_v = 1:size(B_temp,1)
            if (B_temp(ind_v))
                tac = X_temp * B_temp(ind_v);
                % Patlak plot
                y = tac./fPET.quant.T.rr;
                x = (cumsum(fPET.quant.T.rr)*fPET.framelength/60)./fPET.quant.T.rr;
                tstar = round(length(x)*fPET.quant.tstar);
                coeff = polyfit(x(tstar:end), y(tstar:end), 1);
                R.d_re(ind_v) = coeff(1);
            end
