...
if fPET.Y.incomplete == 1
    ...
    if (fPET.glm.X.incomplete.start(1) ~= 1) && (fPET.glm.X.incomplete.no_anchor == 0)
        x(1) = [];
    end
end

% set type of quantification: AIF Patlak, Refregion Patlak
if isfield(fpetbatch.quant.in,'type') && ~isempty(fpetbatch.quant.in.type)
    fPET.quant.in.type = fpetbatch.quant.in.type;
else
    fPET.quant.in.type = fpet_defaults.quant.in.type;
end

% AIF Patlak plot
if (fPET.quant.in.type == 1) || (fPET.quant.in.type == 3)
    % vb
...
% Reference region Patlak plot
if (fPET.quant.in.type == 2) || (fPET.quant.in.type == 3)
    Y.d = spm_read_vols(fPET.Y.h);
    Y.d(isinf(Y.d)) = NaN;

    % mask for reference region
    M.rr.h = spm_vol(fpetbatch.quant.in.mask.rr);
    M.rr.d = spm_read_vols(M.rr.h);
    M.rr.d(isinf(M.rr.d)) = 0;
    M.rr.d(isnan(M.rr.d)) = 0;
    M.rr.d(M.rr.d~=0) = 1;
    M.rr.d = logical(M.rr.d);
    fPET.quant.M.rr = M.rr;
    
    % reference TAC definition
    T.rr = zeros(size(Y.d,4),1);
    for ind = 1:size(Y.d,4)
        temp = Y.d(:,:,:,ind);
        T.rr(ind,1) = nanmean(temp(M.rr.d));
    end
    clear temp;
    T.rr(isnan(T.rr)) = 0;
    fPET.quant.T.rr = T.rr;
end
