...
% Copyright (C) 2024, Neuroimaging Labs, Medical University of Vienna, Austria
    ...
    switch selectedNode.Text
        ...
        case 'Basic Connectivity Settings'
            handles.Menu = 8;
            updateTable(handles.Table, {'Run Connectivity [*]', convertBinaryToTrueFalse(fpetbatch.run_conn); 'Input Data [*]', covertNiiToDesc(fpetbatch.conn.in.data);'Baseline Removal Technique [*]', convertValtoConn(fpetbatch.conn.in.type); 'Start point for Baseline removal', num2str(fpetbatch.conn.in.bl_start_fit); 'Atlas', fpetbatch.conn.in.atlas; 'Motion Regressors', fpetbatch.conn.in.regr_motion; 'PCA Regressors', convertBinaryToTrueFalse(fpetbatch.conn.in.regr_motion_pca); 'Additional Regressors', convertValtoPad(fpetbatch.conn.in.regr_add); 'Baseline Mask [*]', covertNiiToDesc(fpetbatch.conn.in.mask_bl); 'Frame Length [*]', num2str(fpetbatch.conn.in.framelength); 'Temporal input [*]', convertValtoName(fpetbatch.conn.in.time); 'WM Nuisance Mask [*for CompCor]', covertNiiToDesc(fpetbatch.conn.in.mask_wm); 'CSF Nuisance Mask [*for CompCor]', covertNiiToDesc(fpetbatch.conn.in.mask_csf)}); 
        case 'Advanced Connectivity Settings'
            handles.Menu = 9;
            updateTable(handles.Table, {'Remove Inital Frames', num2str(fpetbatch.conn.in.rem_start); 'Remove Final Frames', num2str(fpetbatch.conn.in.rem_end); 'Pad incomplete motion regressors at', convertValtoPad(fpetbatch.conn.in.regr_motion_incomplete); 'Nuisance data', covertNiiToDesc(fpetbatch.conn.in.data_norm); 'Threshold for tissue masks', num2str(fpetbatch.conn.in.nui_t); 'Calculation mask', covertNiiToDesc(fpetbatch.conn.in.mask_calc); 'Cutoff Frequency', [num2str(fpetbatch.conn.in.fil.cutoff(1)) ' - ' num2str(fpetbatch.conn.in.fil.cutoff(2))]; 'Sigma(t)', num2str(fpetbatch.conn.in.fil.sig_t); 'Sigma(s)', num2str(fpetbatch.conn.in.fil.sig_s)}); 
        case 'Covariance'
            ...
            handles.Menu = 10;
            updateTable(handles.Table, {'Run Covariance [*]', convertBinaryToTrueFalse(fpetbatch.run_cov); 'Input Data [*]', tmp; 'Atlas [*]', fpetbatch.cov.in.atlas; 'Normalization Mask [*]', fpetbatch.cov.in.mask_norm; 'Regressors', fpetbatch.cov.in.regr_add; 'Run Jackknife decomposition', convertBinaryToTrueFalse(fpetbatch.cov.in.jk); 'Run PCA decomposition', convertBinaryToTrueFalse(fpetbatch.cov.in.pca); 'Number of PCs', num2str(fpetbatch.cov.in.pc); 'Run ICA decomposition', convertBinaryToTrueFalse(fpetbatch.cov.in.pca); 'Number of ICs', num2str(fpetbatch.cov.in.ic)});
        case 'ICA'
